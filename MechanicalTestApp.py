...
def getInformations(csv_file):
    ...
    my_reader = bytes_data.decode("utf-8")
    t= np.zeros([len(my_reader)-11, 6])
    i=0 # Row index
    ...
    return  df_SpecimenSpecs, df_data

def v2_1(df_SpecimenSpecs, df_data): 
    #Extracting data from both DataFrame 
    ...
    # with col1:
    #     if 'Modev2_1' not in st.session_state:            
    #         st.session_state.Modev2_1 = 1  
    #     def Modev2_1_update():
    #         if 'Modev2_1' not in st.session_state:
    #             st.session_state.Modev2_1 = 1 
    #         else:
    #             if st.session_state.Modev2_1 == 0:
    #                 st.session_state.Modev2_1 = 1
    #             elif st.session_state.Modev2_1 == 1 :
    #                 st.session_state.Modev2_1 = 0
    #         return
            
    #     st.button(Answer[st.session_state.Modev2_1], on_click= Modev2_1_update())

    #     Default = Answer[(st.session_state.Modev2_1+1)%2]
    ...
    if Default=='No':
        ...
        ###################### Streamlit initialisation des widgets #################
        ...
        ######### widget Tear Load ###########
        colMSTL,colMSTL1 = st.columns([1,3])
        with colMSTL:
            MSTL = st.select_slider('Manually set tear load ?',options=['Yes','No'])
        with colMSTL1:
            st.write("")
            with st.expander("Why?"):
                st.write("explanations to add")
    ...
    #         plt.show()
            ...
            spi = st.slider("Choose the index of the value where you want the tear load to be",min_value = 0, max_value= len(x))
...
def Codev2_1(uploaded_file):
    t=np.array(0)  
    df_SpecimenSpecs, df_data = getInformations(uploaded_file)
    with st.container():
        st.header("Data Processing v2.1")
        st.write(df_SpecimenSpecs) #.style.hide_index().format(decimal='.', precision=2).to_html(), unsafe_allow_html= True)
        v2_1(df_SpecimenSpecs, df_data)
    return
...
def main():
    ...
    with Version_col:
        VersionList = ("v0", "v0.01" , "v0.1", "v2" , "v2.1")
        VersionIndex = 4
